            ...
            <div class="bg-bg-panel border border-border-soft p-8 relative">
                ...
                <form action="{{ route('contact.send') }}" method="POST" class="space-y-5">
                    ...
                    <div>
                        ...
                        @error('name')
                            <p class="text-red-400 font-mono text-[11px] mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="message" rows="5" required placeholder="Décrivez brièvement votre besoin ou projet..."
                            class="w-full bg-bg-dark border border-border-soft px-4 py-3 text-gray-200 font-mono text-sm rounded-none focus:border-accent focus:outline-none transition-all resize-none placeholder:text-gray-600">{{ old('message') }}</textarea>
                        @error('message')
                            <p class="text-red-400 font-mono text-[11px] mt-1">{{ $message }}</p>
                        @enderror
                    </div>
